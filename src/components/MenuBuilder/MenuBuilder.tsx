...
import type { FormData, Menu, MenuItemType } from '../../types'
...
export function MenuBuilder({ internalCollections = [], maxDepth: _maxDepth = 3 }: Props) {
  ...
  const handleAdd = async () => {
    if (!navigationHandle || !formData.title) {
      return
    }
    setLoading(true)
    ...
    try {
      ...
      if (formData.type === 'url') {
        ...
        if (!formData.passive) {
          toast.error('Passive label is required')
          setLoading(false)
          return
        }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
      <h4 style={{ color: 'var(--theme-elevation-0)', fontSize: '0.875rem', margin: 0 }}>
        Add Menu Item
      </h4>
      ...
      {formData.type === 'internal' && internalCollections.length > 0 && (
        <InternalField
          internalCollections={internalCollections}
          onChange={(v) =>
            setFormData({ ...formData, internal: v?.value ?? '', relationTo: v?.relationTo ?? '' })
          }
          value={
            formData.internal
      ...
      <Button disabled={loading || !formData.title} onClick={handleAdd} type="button">
